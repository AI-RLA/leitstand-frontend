...
import type { NavigationStageInput } from "@/api/client";
...
function emptyStage(frame: WaypointFrame = "wgs84"): StageDraft {
  return {
    kind: "navigation",
    frame,
    site_id: "",
...
export function MissionNew() {
  ...
  function setStageKind(si: number, kind: StageKind) {
    // Today only "navigation" exists; extending this requires a per-kind
    // empty-stage builder so the body shape matches the new kind.
  ...
  function setStageFrame(si: number, frame: WaypointFrame) {
    ...
    // Switching frame invalidates any in-progress add mode for this stage.
    if (addingIndex === si) setAddingIndex(null);
  }
  ...
  const handleMapClick = useCallback(
    (lat: number, lon: number) => {
      ...
      setStages((prev) => {
        ...
        // If the last waypoint is empty, replace it; else append.
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    navigate({ to: "/missions/$id", params: { id: mission.mission_id } });
  }
      ...
      <div className="w-[520px] shrink-0 flex flex-col border-r border-border bg-white">
        ...
        {create.isError && (
          <p className="text-ui-sm text-red-500 px-4 py-2 border-t border-border bg-red-50">
            Failed to save mission. Check that all fields are valid.
          </p>
        )}
      </div>
